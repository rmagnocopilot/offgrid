...
export class ContextManager {
  ...
  updateActive(editor = vscode.window.activeTextEditor): void {
    ...
    this.autoFile = vscode.workspace.asRelativePath(editor.document.uri, true);
  }
  ...
  priority(prompt: string): string[] {
    const editor = vscode.window.activeTextEditor;
    const selectionFile = editor && !editor.selection.isEmpty ? vscode.workspace.asRelativePath(editor.document.uri, true) : undefined;
